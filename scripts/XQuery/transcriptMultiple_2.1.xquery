...
let $textpart := $iData//TEI:div[@type="textpart"][child::element()/name()='ab' or child::element()/name()='lg'](:[exists(@corresp)]:)
...
let $body3 := substring-before($tail2, '_R') 
...
(:
let $transcriptFile := $corpusTranscript//file[data(@name)=$TM] 
:)

(:

for $file in $corpusTranscript//file 
...
let $fileNameOutput := to:substring-before-match($fileNameAlto,'.xml')
return 
file:write(concat("file:///", $destinationAUX, $fileNameOutput, '_tF_3.4',".xml"), $transcriptFile)
:)
...
let $body3 := substring-before($tail2, '_R.png') 
let $pos3 := to:substring-after-last-match($body3, '_')
return (
    <side>{
    if (matches($pos1,'[rv]'))
...
(:    <sectionArab>{
    ...
    let $n := to:get-matches(to:substring-after-match($info, '(column|fragment|folio)'),'[ABC\d]+[/\-]?[ABC\d]*')
        
    return 
    (<subtype n="{count(to:get-matches($info, '(column|fragment|folio)'))}">{$subtype}</subtype>,       (: @n ist hier die Anzahl der Angaben in Spalte 'TextPart' der Tabelle :)
    ...
    return <n n="{$posN}">{if (matches($nInfo,'-')) 
        then (<from>{to:substring-before-match($nInfo,'-')}</from>,<to>{to:substring-after-match($nInfo,'-')}</to>)
            ...
            then (<div n='1'>{to:substring-before-match($nInfo,'/')}</div>,<div n='2'>{to:substring-after-match($nInfo,'/')}</div>)
            else $nInfo
...
let $matchEntry :=
    ...
    if (empty($t/element()))
        ...
        then let $name := $t//metaName/data(@source) 
             ...
             return <match source='{$t/data(@n)}' name='{$name}' type='{$type}'><meta>{$t//metaTEI,$t//metaName}</meta>{$textparts}</match>
             
        else (: if (not(count($t//textpartMeta//metaTEI)=1 and count($t//textpartMeta//metaName)=1)) 
            then :)
            ...
            for $metaName in $cohesive//metaName return
                if (matches(to:substring-after-last($metaName/data(@source),'_'),'\d+\-\d+'))  (: Kriterium true, wenn Bild aufgesplittet wurde und dem Dateinamen die Zeilenanzahl im Stil "0-1900" hinzugefügt wurde :)
                    then let $name := $metaName/data(@source) 
                         let $type := '2.0' 
                         let $textparts := $cohesive//textpart[xs:int($metaName/data(@n))]
...
let $fileNameOutput := to:substring-before-match($fileNameAlto,'.xml')
return (
file:write(concat("file:///", $destinationAUX, $fileNameOutput, '_tT_3.4',".xml"), $transcriptTextpart),
file:write(concat("file:///", $repository, $destinationAlto, $fileNameOutput, '_match', '.xml'), $match)
)
:)
...
let $Normalised := <norm>{for $textpart in $match//textpart return <textpart>{for $textpartN in $textpart//text[data(@editionType)='normalized']//line[child::token or child::unclear or child::supplied] return $textpartN}</textpart>}</norm>     
let $Diplomatic := <dipl>{for $textpart in $match//textpart return <textpart>{for $textpartD in $textpart//text[data(@editionType)='diplomatic']//line[child::token or child::unclear or child::supplied] return $textpartD}</textpart>}</dipl>
...
file:write(concat("file:///", $repository, $destinationAlto, $version, '/', $fileNameOutput, '.xml'), $alto[$pos])



(:file:write(concat("file:///", $repository, $destinationAlto, $fileNameOutput, '(d1)', '.xml'), $copyD1), :)
...
(:
(
file:write(concat("file:///", $repository, $destinationAlto, $fileNameOutput, '(d2)', '.xml'), $copyD2),
file:write(concat("file:///", $repository, $destinationAlto, $fileNameOutput, '(d3)', '.xml'), $copyD3), 
file:write(concat("file:///", $repository, $destinationAlto, $fileNameOutput, '(d4)', '.xml'), $copyD4), 
file:write(concat("file:///", $repository, $destinationAlto, $fileNameOutput, '(n1)', '.xml'), $copyN1),
file:write(concat("file:///", $repository, $destinationAlto, $fileNameOutput, '(n2)', '.xml'), $copyN2),
file:write(concat("file:///", $repository, $destinationAlto, $fileNameOutput, '(n3)', '.xml'), $copyN3),
file:write(concat("file:///", $repository, $destinationAlto, $fileNameOutput, '(n4)', '.xml'), $copyN4)  
)
:)
